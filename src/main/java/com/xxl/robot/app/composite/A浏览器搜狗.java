...
public class A浏览器搜狗 {
    ...
    public static void handle(Robot robot,String robotCode, String appCode, String event,List<PhoneCodeDto> phoneCodeDtos){
        ...
        switch (event){
            ...
            case AppConstants.SEARCH://搜索
                handle20(robot, robotCode, appCode, event, phoneCodeDtos, apps);
                break;
        }
   ...
   /*     AppUserTools.handle(robot, robotCode,  appCode,  event, phoneCodeDtos, apps);
     ...
     * @param phoneCodeDtos
     */
    public static void handle20(Robot robot,String robotCode, String appCode, String event,List<PhoneCodeDto> phoneCodeDtos,List<AppDto> apps){
        for(int i=0;i<5;i++) {
            AppDto  app = new AppDto();
            app.setCategory("category用户中心");
            app.setEvent("category用户中心-活动");
