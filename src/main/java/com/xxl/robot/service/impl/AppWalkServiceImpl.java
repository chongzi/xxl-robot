...
//************************一种类型：签到********************************************************
	...
	public void start(String robotCode){
		...
		A步行多多步.handle(robot,robotCode,"多多步", AppConstants.CHECK_IN,dtos);

		log.info("********************步数赚**************************");
		List<PhoneCodeDto> dtos1 = phoneCodeService.getList(robotCode,"步数赚");
		A步行多多步.handle(robot,robotCode,"步数赚", AppConstants.CHECK_IN,dtos1);

		log.info("********************步步宝赚**************************");
		List<PhoneCodeDto> dtos2 = phoneCodeService.getList(robotCode,"步步宝");
		A步行步步宝.handle(robot,robotCode,"步步宝", AppConstants.CHECK_IN,dtos2);
		...
		log.info("********************步步赚钱**************************");
		List<PhoneCodeDto> dtos6 = phoneCodeService.getList(robotCode,"步步赚钱");
		A步行步步赚钱.handle(robot,robotCode,"步步赚钱", AppConstants.CHECK_IN,dtos6);


		log.info("********************一起来走路**************************");
		List<PhoneCodeDto> dtos7 = phoneCodeService.getList(robotCode,"一起来走路");
		A步行一起来走路.handle(robot,robotCode,"一起来走路", AppConstants.CHECK_IN,dtos7);
...
//************************二种类型：分段（一次性收取，睡觉收取，吃饭，喝水，打卡，种菜，分享，游戏，充电，步行收取）*****************************************
	...
	public void  section4(String robotCode){
		...
		log.info("********************多多步**************************");
		List<PhoneCodeDto> dtos = phoneCodeService.getList(robotCode,"多多步");
		A步行多多步.handle(robot,robotCode,"多多步", AppConstants.WALK,dtos);
...
//*************************三种类型：循环(开宝箱，看广告，领红包,看视频，看新闻，看小说，刮卡，抽奖)********************************************************
	...
	public void circulate1(String robotCode){
		...
		log.info("********************多多步**************************");
		List<PhoneCodeDto> dtos = phoneCodeService.getList(robotCode,"多多步");
		A步行多多步.handle(robot,robotCode,"多多步", AppConstants.WATCH_ADVERT,dtos);
	    ...
	    log.info("********************步步宝赚**************************");
		List<PhoneCodeDto> dtos2 = phoneCodeService.getList(robotCode,"步步宝");
		A步行步步宝.handle(robot,robotCode,"步步宝", AppConstants.GIFT_MONEY,dtos2);
		A步行步步宝.handle(robot,robotCode,"步步宝", AppConstants.TREASURE,dtos2);


		log.info("********************走走赚**************************");
		List<PhoneCodeDto> dtos3 = phoneCodeService.getList(robotCode,"走走赚");
		A步行走走赚.handle(robot,robotCode,"走走赚", AppConstants.TREASURE,dtos3);
		A步行走走赚.handle(robot,robotCode,"走走赚", AppConstants.WATCH_ADVERT,dtos3);
		A步行走走赚.handle(robot,robotCode,"走走赚", AppConstants.GIFT_MONEY,dtos3);
		A步行走走赚.handle(robot,robotCode,"走走赚", AppConstants.DRINK_WATER,dtos3);
		...
		A步行步多多.handle(robot,robotCode,"步步赚钱", AppConstants.GIFT_MONEY,dtos6);

		log.info("********************一起来走路**************************");
		List<PhoneCodeDto> dtos7 = phoneCodeService.getList(robotCode,"一起来走路");
		A步行一起来走路.handle(robot,robotCode,"一起来走路", AppConstants.WATCH_ADVERT,dtos7);
		A步行一起来走路.handle(robot,robotCode,"一起来走路", AppConstants.GIFT_MONEY,dtos7);
