...
/**
 * todo 添加群（获取数据库中数据发送，截取window中程序中内容，实现模拟人工操作电脑）
 */
public class AddTools {
    private static final org.slf4j.Logger log = LoggerFactory.getLogger(AddTools.class);
     ...
     *  param dbData     要发送的数据
     */
    public static void QQAdd(String windowName,String qq) {
        try {
            ...
            if (bool) {
                ...
                System.out.println("QQ自动添加群发送数据******************");
            ...
            log.info("QQSend--QQ自动添加群出现异常,当前窗口windowName标题为：{}"+windowName);
            e.printStackTrace();
        }
    }
